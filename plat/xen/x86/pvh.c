			     ...
			     struct pvh_start_info *pi)
{
	...
	if (unlikely(rc < 0))
		UK_CRASH("Unable to add cmdline mapping", rc);
}
			    ...
			    struct pvh_start_info *pi)
{
	...
	for (i = 0; i < pi->nr_modules; i++) {
		...
		mrd.vbase = mrd.pbase;
		mrd.len = PAGE_ALIGN_UP(mod->size);
		mrd.pg_off = 0;
		mrd.pg_count = PAGE_COUNT(mrd.pg_off + mrd.len);
		...
		rc = ukplat_memregion_list_insert(&bi->mrds, &mrd);
		if (unlikely(rc < 0))
			UK_CRASH("Unable to add initrd mapping", rc);
	}
}
			    ...
			    struct pvh_start_info *pi)
{
	...
	for (i = 0; i < pi->memmap_entries; i++) {
		...
		rc = ukplat_memregion_list_insert(&bi->mrds, &mrd);
		if (unlikely(rc < 0))
			UK_CRASH("Unable to add ram mapping", rc);
	}
}
...
void libxenplat_start(struct lcpu *lcpu, struct pvh_start_info *pi)
{
	...
	if (unlikely(!bi))
		UK_CRASH("Incompatible or corrupted bootinfo", -EINVAL);

	if (pi->magic != PVH_START_MAGIC_VALUE)
		UK_CRASH("Invalid magic number", -EINVAL);
